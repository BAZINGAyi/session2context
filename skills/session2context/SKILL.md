...
<项目根>/
└── s2c-data/
    ├── sessions/       ← 第一层：原始 session 摘要（用户确认后写入，可删，低信噪比）
    └── preferences/    ← 第二层：提炼后的偏好（经用户确认写入，传给 LLM，高信噪比）
...
```
Step 1  判断 session 类型（coding / general）
  ↓
Step 2  生成摘要草稿 → 展示给用户确认 → 写入 s2c-data/sessions/
  ↓
Step 3  询问是否提炼到 preferences/
  ├─ 用户回「否」→ 流程结束（不执行 Step 4 / Step 5）
  └─ 用户回「是」或选编号 ↓
Step 4  逐条提炼 → 展示变更预览 → 用户确认 → 写入 preferences/
  ↓（有任何文件写入时才继续）
Step 5  刷新 Agent 指令配置（调用脚本注入描述）
  ↓
结束
```

> **每个需要写入的步骤都必须先获得用户明确确认，才能执行写入。**
...
写入时的两项检查（在用户确认后、实际写入前执行）：
1. **目录不存在**：若 `s2c-data/sessions/` 不存在，先创建该目录再写入。
2. **文件已存在**：若同名文件已存在，告知用户并询问处理方式：
   - **覆盖**：直接覆盖原文件
   - **重命名**：在文件名末尾追加 `-2`（依此类推：`-3`、`-4`…）
   - **取消**：放弃本次写入
...
回复 **"是"或选择条目编号** → 继续 Step 4。
回复 **"否"** → 流程结束，不执行 Step 4 和 Step 5。
```
...
2. 为上一步返回的**每个**文件生成一行描述：先**通读文件内容**（忽略「加载自检」行，不纳入描述），再以 `##` 小节名为骨架，概括出该文件涵盖什么 + 加载时机。要求**含义完整、不遗漏关键类别**，同时尽量精炼，不逐条复述细节（必要时可在括号里点到关键点）。格式：
   ...
   > **⚠️ 加载时机是真正的条件触发器，不是注释。** LLM 读到这行描述时，会依据末尾的时机决定是否读取该文件，而不是无条件全量加载。因此：
   > - 描述末尾必须写明确的触发条件，例如："所有对话必读"、"涉及软件设计/架构/代码规范时必读"、"使用 <工具名> 时必读"
   > - **不要对所有文件都写"必读"**——只有真正每次对话都需要的文件才用"所有对话必读"；有明确适用场景的文件要写具体条件
